...
export const ExpenseManager: React.FC<ExpenseManagerProps> = ({ role, initialExpenses }) => {
  ...
  // --- LOGIKA HAK AKSES ---
  // Berdasarkan requirement: Pegawai hanya bisa INSERT (input only)
  // Owner bisa INSERT, UPDATE, DELETE
  ...
  const handleSaveEdit = async (exp: ExpenseRow) => {
    ...
    if (!canEdit) {
      alert("Hanya Owner yang dapat mengedit data keuangan.")
      return
    }
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        new Date(r.tanggal_pengeluaran)
  ...
  return (
    <>
      <form
        ...
        className="bg-white p-6 md:p-8 rounded-2xl shadow-lg max-w-3xl mx-auto"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm mb-1 text-(--color-dark-primary)">
              Tanggal
            </label>
            <input
              type="date"
            ...
            <tbody>
              {filteredRows.map((r) => (
                <tr key={r.expense_id} className="border-b">
                  <td className="p-3">
                    {new Date(r.tanggal_pengeluaran).toLocaleDateString(
                      "id-ID"
                    )}
                  </td>
                  <td className="p-3">
