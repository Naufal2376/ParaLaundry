...
export default function UpdateStatusPage() {
  ...
  const [currentStatus, setCurrentStatus] =
    useState<StatusCucian>("Masuk Antrean")
  ...
  const fetchOrder = async (inputCode: string) => {
    ...
    try {
      ...
      if (error || !data) {
        ...
        const customerName = Array.isArray(fetchedData.customer)
          ? fetchedData.customer[0]?.nama ?? "Tanpa Nama"
          : fetchedData.customer?.nama ?? "Tanpa Nama"
        const orderItems = (fetchedData.order_details || []).map((d: any) => ({
          nama_layanan: Array.isArray(d.service)
            ? d.service[0]?.nama_layanan ?? "N/A"
            : d.service?.nama_layanan ?? "N/A",
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-md mb-6">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-3 bg-blue-600 text-white rounded-lg flex items-center justify-center gap-2 hover:bg-blue-700 transition duration-200 disabled:bg-gray-400"
          >
            {loading ? (
          ...
          <button
            ...
            className="px-5 py-3 bg-white text-blue-600 border-2 border-blue-600 rounded-lg flex items-center justify-center gap-2 hover:bg-blue-50 transition duration-200"
          >
      ...
      {order && (
        <div className="bg-white p-6 rounded-2xl shadow-lg animate-fade-in">
          ...
          <div>
            <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
              <CheckCircle size={18} /> Perbarui Status
            </h3>
            <div className="flex flex-col sm:flex-row items-stretch gap-3">
              <select
                value={currentStatus}
                onChange={(e) =>
                  setCurrentStatus(e.target.value as StatusCucian)
                }
